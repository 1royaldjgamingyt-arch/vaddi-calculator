...
fun CalculationResult.getLocalizedDuration(context: android.content.Context): String {
    return when (mode) {
        CalculatorMode.QUICK -> {
            ...
            when (interestType) {
                InterestType.MONTHLY -> "$d ${context.getString(com.vaddicalculator.app.R.string.unit_months)}"
                InterestType.YEARLY -> "$d ${context.getString(com.vaddicalculator.app.R.string.unit_years)}"
                InterestType.DAILY -> "$d ${context.getString(com.vaddicalculator.app.R.string.unit_days)}"
            }
        }
        CalculatorMode.DATE_BASED -> {
            if (durationBreakdown != null) {
                "${durationBreakdown.toLocalizedReadableString(context)} ($totalDays ${context.getString(com.vaddicalculator.app.R.string.unit_days)})"
            } else {
                durationDescription
            }
        }
...
fun DurationBreakdown.toLocalizedReadableString(context: android.content.Context): String {
    val parts = mutableListOf<String>()
    if (years > 0) {
        parts.add("$years ${context.getString(com.vaddicalculator.app.R.string.unit_years)}")
    }
    if (months > 0) {
        parts.add("$months ${context.getString(com.vaddicalculator.app.R.string.unit_months)}")
    }
    if (days > 0) {
        parts.add("$days ${context.getString(com.vaddicalculator.app.R.string.unit_days)}")
    }
    return if (parts.isEmpty()) {
        "0 ${context.getString(com.vaddicalculator.app.R.string.unit_days)}"
    } else {
        parts.joinToString(", ")
    }
}
